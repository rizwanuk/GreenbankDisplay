...
export function getCurrentPrayerState({
  ...
  if (!todayRow || !settings) {
    return { key: "none", label: "", arabic: "", start: null, jamaah: null, isMakrooh: false, inJamaah: false };
  }
  ...
  let state = { key: "none", label: "", arabic: "", start: null, jamaah: null, isMakrooh: false, inJamaah: false };
  const set = (key, label, arabic, start = null, jamaah = null, isMakrooh = false) => {
    state = { key, label, arabic, start, jamaah, isMakrooh, inJamaah: false };
  };
  ...
  if (isTime(fajrStart) && now.isBefore(fajrStart)) {
    ...
    set("fajr", labels?.fajr, arabicLabels?.fajr, fajrStart, fajrJamaah);
  } else if (isTime(fajrJamaah) && now.isSameOrAfter(fajrJamaah) && now.isBefore(fajrJamaah.clone().add(jamaahDurationMin, "minutes"))) {
    state = { key: "fajr", label: labels?.fajr, arabic: arabicLabels?.fajr, start: fajrStart, jamaah: fajrJamaah, isMakrooh: false, inJamaah: true };
    ...
    set("makrooh", `Makrooh ${arabicLabels?.makrooh || "مكروه"} time — please avoid praying`, null, null, null, true);
    ...
    set("ishraq", `Ishraq ${arabicLabels?.ishraq || "اشراق"}`, null);
    ...
    state = { key: isFriday ? "jummah" : "dhuhr", label: isFriday ? (labels?.jummah || "Jum‘ah") : labels?.dhuhr, arabic: isFriday ? arabicLabels?.jummah : arabicLabels?.dhuhr, start: dhuhrStart, jamaah: dhuhrJamaah, isMakrooh: false, inJamaah: true };
    ...
    state = { key: "asr", label: labels?.asr, arabic: arabicLabels?.asr, start: asrStart, jamaah: asrJamaah, isMakrooh: false, inJamaah: true };
    ...
    state = { key: "maghrib", label: labels?.maghrib, arabic: arabicLabels?.maghrib, start: maghribStart, jamaah: maghribJamaah, isMakrooh: false, inJamaah: true };
    ...
    state = { key: "isha", label: labels?.isha, arabic: arabicLabels?.isha, start: ishaStart, jamaah: ishaJamaah, isMakrooh: false, inJamaah: true };
  } else {
