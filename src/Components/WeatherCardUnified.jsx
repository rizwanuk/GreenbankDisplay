...
function readNumber(key) { try { const v = Number(localStorage.getItem(key)); return Number.isFinite(v) ? v : 0; } catch { return 0; } }
function writeNumber(key, n) { try { localStorage.setItem(key, String(n)); } catch {} }

function getCache(cacheKey) { return readJSON(keyForCache(cacheKey)); }
function setCache(cacheKey, value) { writeJSON(keyForCache(cacheKey), value); }

function readRateLimitUntil() { return readNumber(keyRateLimit) || 0; }
function writeRateLimitUntil(untilTs) { writeNumber(keyRateLimit, untilTs || 0); }
...
function parseNextAccessTime(val) {
  ...
  return new Date(Date.UTC(Number(y), month, Number(d), Number(hh), Number(mm), Number(ss))).getTime();
}

export default function useMetOfficeWeather({ lat, lon, mode = "3h", dailyBudget = DEFAULT_DAILY_BUDGET }) {
  // Normalize incoming settings right away so NaN doesn't slip through
  ...
  const urlsToTry = useMemo(() => {
    ...
    const order = preferredKind === "hourly" ? ["hourly", "three-hourly", "daily"] : ["three-hourly", "daily"];
    ...
    if (!force) {
      for (const { kind, url } of urlsToTry) {
        ...
        if (cached && now - (cached.savedAt || 0) < ttlFor(kind)) {
          ...
          // soft background refresh (non-blocking) to keep cache warm
          (async () => {
            try { await ensureMinInterval(); await doNetwork(kind, url); } catch {}
          })();
          return;
        }
      }
    ...
    try {
        ...
        try {
          ...
          if (err?.status === 429) {
            ...
            for (const { kind: k2, url: u2 } of urlsToTry) {
              ...
              if (cached) {
                setState({ loading: false, data: cached.data, error: { type: "rate-limit", until: untilTs, raw: err?.raw }, resolvedKind: k2 });
                return;
              }
            }
            setState({ loading: false, data: null, error: { type: "rate-limit", until: untilTs, raw: err?.raw }, resolvedKind: null });
            return;
          }
          ...
          // For 400 or any other error: show stale cache if present
          const cached = getCache(url);
          if (cached) {
            setState({ loading: false, data: cached.data, error: { type: "http", status: err?.status || 0, message: err?.message, raw: err?.raw }, resolvedKind: kind });
            return;
          }
      ...
      if (outer?.name === "ClientBudgetError") {
        for (const { kind, url } of urlsToTry) {
          const cached = getCache(url);
          if (cached) {
            setState({ loading: false, data: cached.data, error: { type: "client-budget", until: outer.until }, resolvedKind: kind });
            return;
          }
        }
        setState({ loading: false, data: null, error: { type: "client-budget", until: outer.until }, resolvedKind: null });
        return;
      }
      if (outer?.name === "RateLimitError") {
        ...
        for (const { kind, url } of urlsToTry) {
          const cached = getCache(url);
          if (cached) {
            setState({ loading: false, data: cached.data, error: { type: "rate-limit", until }, resolvedKind: kind });
            return;
          }
        }
        setState({ loading: false, data: null, error: { type: "rate-limit", until }, resolvedKind: null });
        return;
      }

      setState({ loading: false, data: null, error: { type: "http", status: outer?.status || 0, message: outer?.message || "Request failed", raw: outer?.raw }, resolvedKind: null });
    }
  };
    ...
    if (first.current) {
      ...
      for (const { kind, url } of urlsToTry) {
        ...
        if (cached && now - (cached.savedAt || 0) < ttlFor(kind)) {
          ...
          (async () => { try { await ensureMinInterval(); await doNetwork(kind, url); } catch {} })();
          return;
        }
      }
    }
