...
function JamaahBanner({ label, arabic, theme = {} }) {
  return (
    <div
      ...
      className={`rounded-xl px-6 py-6 text-center shadow ${theme?.jamaahColor || 'bg-green-700'} ${theme?.textColor || 'text-white'}`}
    >
      <div className="flex items-center gap-4 justify-center mb-2">
        <span className={`${theme?.nameSize || 'text-6xl sm:text-7xl md:text-8xl'} font-eng font-semibold`}>
          {label}
        </span>
        {arabic && (
          <span
...
export default function CurrentPrayerCard({
  ...
  if (!todayRow || !settingsMap) {
    return (
      <div
        style={toFontVars(theme)}
        className={`rounded-xl px-4 py-6 text-center ${theme?.bgColor || 'bg-white/5'} ${theme?.textColor || 'text-white'}`}
      >
        Times are loading…
      </div>
    );
