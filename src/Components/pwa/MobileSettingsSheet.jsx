...
/** Discover theme names from Google Sheet rows:
 *  Expect Group like: "themeMobile.Theme_4.currentPrayer"
 */
function getMobileThemeNames(rows) {
  ...
  for (const r of rows) {
    ...
    const parts = g.split("."); // ["themeMobile","Theme_4","currentPrayer"]
...
export default function MobileSettingsSheet({
  ...
  onClose,
  settingsRows,
  settings,             // optional structured settings object
  ...
  // Coerce settingsRows to an array in case a wrapper object was passed
        ...
        <div className="px-4 py-4 grid gap-4">
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.06] p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {themeNames.length === 0 && (
                <div className="text-white/70 text-sm">No mobile themes found in Settings sheet.</div>
              )}
              {themeNames.map((name) => (
                <button
                  key={name}
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.06] p-4">
            ...
            <div className="flex flex-wrap gap-2">
              <button onClick={onCheckUpdate} disabled={busy} className="px-3 py-1.5 rounded-lg border border-white/15 bg-white/10 hover:bg-white/15 disabled:opacity-60">
                Check for update
              </button>
              <button onClick={onApplyUpdate} disabled={busy} className="px-3 py-1.5 rounded-lg border border-white/15 bg-white/10 hover:bg-white/15 disabled:opacity-60">
                Apply update
              </button>
              {/* Install PWA */}
